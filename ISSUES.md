...
- **BG chr base shift** — was already correct (`<< 13` for BGs, `<< 14` for sprites)
...
The emulator now has a real SPC700, DSP, and audio output path, but the audio side is still not cycle-accurate. Super Mario World music behavior is still under investigation; current suspects are remaining CPU↔SPC protocol nuances and incomplete DSP timing/pipeline behavior.

### 2. Modes 2-6 are still simplified
**File:** `PPU.swift`
**Severity:** Medium

Modes 2-6 are not yet modeled with their real per-mode behavior. Offset-per-tile, 8bpp backgrounds, and hi-res details are still incomplete.
...
### 4. Color math and sub-screen blending are still missing
**File:** `PPU.swift`
**Severity:** Medium

`CGWSEL`, `CGADSUB`, `COLDATA`, and the sub-screen path are stored but not used to produce additive/subtractive blending.
